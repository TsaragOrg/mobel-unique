...
- render preparation, coverage, validation, and publication;
...
- letting customers choose a fabric and sofa view;
- letting customers launch an in-home simulation from their selected sofa, fabric, and view;
...
- fabric and view selection for a selected sofa;
- in-home simulation from an uploaded customer room photo;
- customer download of the generated simulation result while the result is still retained;
...
- administrator validation before renders become public;
- administrator ZIP export of approved sofa render sets for Shopify-side reuse;
...
- choose a public view for the selected sofa;
- start an in-home simulation from the selected sofa, fabric, and view;
- upload a room photo for that simulation;
- view the simulation result;
- download the generated simulation result while it is still available;
...
- define which views are required for each sofa;
- provide or generate public renders for sofa, fabric, and view combinations;
...
- download approved sofa render sets as ZIP files for Shopify-side reuse;
...
4. The customer sees the default public fabric and default public view for the sofa.
5. The customer can switch fabric and view within the public options available for that sofa.
6. The customer starts an in-home simulation from the selected sofa, fabric, and view.
...
9. The customer can download the generated simulation result while it is still available.
...
5. The customer chooses a public fabric and public view.
6. The customer starts the in-home simulation.
7. The customer can download the generated simulation result while it is still available.
...
3. The administrator prepares sofas and their required public views.
...
6. The administrator validates renders before publication.
7. The administrator publishes only sofas and fabric options that are complete enough for a consistent public experience.
8. The administrator downloads approved sofa render sets as ZIP files when those images are needed for Shopify-side reuse.
9. The administrator copies the public visualization link into Shopify.
...
Its primary purpose is to help the customer choose a fabric and view before launching an in-home simulation.
...
### Sofas, Fabrics, And Views

Sofas and fabrics are separate product concepts.

A fabric is reusable and can be assigned to multiple sofas.

Each sofa has an ordered set of public views. The customer-facing view order must remain stable across public fabrics for that sofa.

When a customer changes fabric, the selected view must keep the same meaning.

### Complete Public Render Coverage

A fabric can be public for a sofa only when that fabric has approved public renders for every required public view of that sofa.

The public experience must not expose partial fabric coverage where some views exist and others are missing.
...
The detailed render matrix, source-photo handling, AI reference selection, regeneration rules, and validation workflow belong in a dedicated render specification.
...
- the first public view according to the administrator-defined order.

Shopify links do not preselect fabric or view in the MVP.

### In-Home Simulation

An in-home simulation always starts from a selected sofa, selected fabric, and selected view.
...
The customer must be able to download the generated simulation result while the result is still retained by the system.
...
- customer email collection is not required;
...
The MVP operational view must remain lightweight. It should focus on selected sofa, selected fabric, selected view, job status, timestamps, failure status, and simple aggregate usage indicators.
...
- the customer selects a public fabric and view;
- the customer launches an in-home simulation from that selection;
- the customer receives a simulation result;
- the customer can download the simulation result while it is still retained;
- the customer can return to Shopify to order;
- an administrator can prepare sofas, fabrics, views, and public renders;
...
- an administrator can download approved sofa render sets as ZIP files for Shopify-side reuse;
...
- Public Customer Experience: a `domain` area covering the public home page, public catalog, sofa detail page, fabric selection, view selection, default states, mobile-first behavior, simulation result access, result download, and Shopify order redirect.
- Admin Catalog And Fabric Management: a `domain` area covering admin sofa preparation, reusable fabric management, sofa metadata, fabric assignment, view definition, Shopify URL entry, draft behavior, publication, and archive behavior.
- Render Preparation, Coverage, Validation, And Publication: a `domain` and `technical` area covering manual uploads, AI-generated renders, render source types, complete coverage rules, validation, publication readiness, regeneration, and required admin ZIP export.
- In-Home Simulation Flow: a `domain` and `technical` area covering room photo upload, guided simulation wizard, room references, dimensions, simulation job lifecycle, result display, result download, retry, and failure states.
...
- Sofas, fabrics, and views are identified as core product concepts.
...
- Customer download of the generated simulation result is included while the result is retained.
...
- Admin ZIP export of approved sofa render sets is included in MVP.
...
- Default public fabric and default public view must use the administrator-defined order.
- The customer must be able to download the generated simulation result while it is still retained.
- Customer room photos and simulation outputs must not be retained for more than 24 hours in the MVP.
- Admin ZIP export of approved sofa render sets is required in MVP because those images may be reused on Shopify.
