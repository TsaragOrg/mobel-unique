...
const scriptPath = fileURLToPath(
  new URL("./in-home-simulation-test-seed-smoke.mjs", import.meta.url)
);
const TEST_TIMEOUT_MS = 45000;

function createFakePsql() {
  const cwd = mkdtempSync(
    join(tmpdir(), "mobel-in-home-simulation-test-seed-smoke-test-")
  );
...
describe("in-home simulation test-seed smoke script", () => {
  it(
    "passes when psql returns no test-seed contract failures",
    async () => {
      const result = await runSmoke({
        IN_HOME_SIMULATION_TEST_SEED_SMOKE_PSQL: createFakePsql()
      });

      expect(result.status).toBe(0);
      expect(result.stdout).toContain(
        "PASS in-home simulation test-seed smoke"
      );
    },
    TEST_TIMEOUT_MS
  );
  ...
  it(
    "fails clearly when psql reports test-seed contract failures",
    async () => {
      const result = await runSmoke({
        FAKE_PSQL_STDOUT:
          "missing helper function: public.seed_in_home_simulation_local_test_job\nseed accepted retention_hours greater than 24\n",
        IN_HOME_SIMULATION_TEST_SEED_SMOKE_PSQL: createFakePsql()
      });

      expect(result.status).toBe(1);
      expect(result.stderr).toContain(
        "FAIL in-home simulation test-seed smoke"
      );
      expect(result.stderr).toContain(
        "missing helper function: public.seed_in_home_simulation_local_test_job"
      );
  ...
  it(
    ...
    async () => {
      const result = await runSmoke({
        ...
        IN_HOME_SIMULATION_TEST_SEED_SMOKE_PSQL: createFakePsql(),
        IN_HOME_SIMULATION_TEST_SEED_SMOKE_TIMEOUT_MS: "50"
      });
      ...
      expect(result.stdout).not.toContain(
        "SKIP in-home simulation test-seed smoke"
      );
    },
    TEST_TIMEOUT_MS
  );
  ...
  it(
    ...
    async () => {
      const result = await runSmoke({
        ...
        IN_HOME_SIMULATION_TEST_SEED_SMOKE_DB_URL:
