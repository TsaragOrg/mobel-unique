...
describe("SPEC-0015 claim RPCs short-circuit on cost-meter pause", () => {
  ...
  it("calls the helper from claim_in_home_simulation_room_prep_job", () => {
    const fn = sql.slice(
      sql.indexOf("create or replace function public.claim_in_home_simulation_room_prep_job"),
      sql.indexOf("create or replace function public.claim_specific_in_home_simulation_room_prep_job")
    );
  ...
  it("calls the helper from claim_specific_in_home_simulation_room_prep_job", () => {
    const fn = sql.slice(
      sql.indexOf("create or replace function public.claim_specific_in_home_simulation_room_prep_job"),
      sql.indexOf("create or replace function public.claim_specific_in_home_simulation_placement_job")
    );
    expect(fn).toContain("if public.simulation_cost_meter_paused() then");
    expect(fn).toContain("return;");
  });
