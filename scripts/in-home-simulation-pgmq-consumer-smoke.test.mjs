...
const scriptPath = fileURLToPath(
  new URL("./in-home-simulation-pgmq-consumer-smoke.mjs", import.meta.url)
);
const TEST_TIMEOUT_MS = 45000;

function createFakePsql() {
  const cwd = mkdtempSync(
    join(tmpdir(), "mobel-in-home-simulation-pgmq-consumer-smoke-test-")
  );
...
describe("in-home simulation pgmq-consumer smoke script", () => {
  it(
    "passes when psql returns no consumer contract failures",
    async () => {
      const result = await runSmoke({
        IN_HOME_SIMULATION_PGMQ_CONSUMER_SMOKE_PSQL: createFakePsql()
      });

      expect(result.status).toBe(0);
      expect(result.stdout).toContain(
        "PASS in-home simulation pgmq-consumer smoke"
      );
    },
    TEST_TIMEOUT_MS
  );
  ...
  it(
    "fails clearly when psql reports consumer contract failures",
    async () => {
      const result = await runSmoke({
        FAKE_PSQL_STDOUT:
          "dequeue did not return exactly the message that was enqueued\nclaim_specific returned a row for a job that was already room_prep_processing\n",
        IN_HOME_SIMULATION_PGMQ_CONSUMER_SMOKE_PSQL: createFakePsql()
      });

      expect(result.status).toBe(1);
      expect(result.stderr).toContain(
        "FAIL in-home simulation pgmq-consumer smoke"
      );
      expect(result.stderr).toContain(
        "dequeue did not return exactly the message that was enqueued"
      );
  ...
  it(
    ...
    async () => {
      const result = await runSmoke({
        ...
        IN_HOME_SIMULATION_PGMQ_CONSUMER_SMOKE_PSQL: createFakePsql(),
        IN_HOME_SIMULATION_PGMQ_CONSUMER_SMOKE_TIMEOUT_MS: "50"
      });
      ...
      expect(result.stdout).not.toContain(
        "SKIP in-home simulation pgmq-consumer smoke"
      );
    },
    TEST_TIMEOUT_MS
  );
  ...
  it(
    ...
    async () => {
      const result = await runSmoke({
        ...
        IN_HOME_SIMULATION_PGMQ_CONSUMER_SMOKE_DB_URL:
