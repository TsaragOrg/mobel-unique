...
const MANIFEST_PATH = resolve(
  process.env.LOCAL_ADMIN_FIXTURE_MANIFEST ??
  ...
  const fabricSlugs =
    arrayOrEmpty(value.fabric_slugs).length > 0
      ? arrayOrEmpty(value.fabric_slugs).map((item, fabricIndex) =>
          requiredString(
            item,
            `sofas[${index}].fabric_slugs[${fabricIndex}]`,
          ),
        )
...
function normalizeVisualPosition(value, path, fallbackSourceFabricSlug) {
  ...
  return {
    adminLabel: stringOrDefault(value.admin_label, `Position ${value.sequence}`),
    publicLabel: stringOrDefault(value.public_label, `Position ${value.sequence}`),
...
async function seedFabrics(manifest) {
  ...
  for (const definition of manifest.fabrics) {
    const swatchImage = readFixtureImage(
      definition.swatchImage,
      `fabric ${definition.slug} swatch`,
    );
    const aiReferenceImage = readFixtureImage(
...
async function seedSofas(manifest, tags, fabrics) {
  ...
  for (const definition of manifest.sofas) {
    ...
    for (const tagSlug of definition.tagSlugs) {
      ...
      if (tag) {
        await upsertRows("sofa_tags", ["sofa_id", "tag_id"], [
          {
            created_at: NOW,
            sofa_id: sofa.id,
            tag_id: tag.id,
    ...
    for (const [index, fabric] of assignedFabrics.entries()) {
      await upsertRows("sofa_fabrics", ["sofa_id", "fabric_id"], [
        {
          assigned_at: NOW,
          fabric_id: fabric.id,
...
async function seedVisualMatrixForSofa({
  ...
  for (const position of definition.visualPositions) {
    ...
    const [sourcePhoto] = await upsertRows(
      "sofa_source_photos",
        ...
        [
          {
            ...
            current_private_asset_id: isSourceFabric ? sourceAsset.id : null,
            current_public_asset_id: null,
            fabric_id: fabric.id,
            sofa_id: sofa.id,
            source_photo_id: isSourceFabric ? sourcePhoto.id : null,
            source_type: isSourceFabric ? "source_photo" : "ai_generated",
            updated_at: NOW,
            visual_matrix_column_id: column.id,
          },
        ],
...
async function upsertImageAsset({
  ...
  await uploadStorageObject(bucketId, objectPath, image.bytes, image.contentType);
...
function readFixtureImage(relativePath, label) {
  if (!relativePath) {
  ...
  if (!existsSync(imagePath)) {
    fail(`${label} image not found at ${imagePath}.`);
  }
...
function defaultManifest() {
  const prefix = process.env.LOCAL_ADMIN_SEED_PREFIX ?? "local-admin";

  return {
    fabrics: [
      {
        internal_name: `${prefix} fabric graphite`,
        is_premium: false,
        public_name: "Local Graphite",
        slug: "graphite",
      },
      {
        internal_name: `${prefix} fabric ivory`,
        is_premium: false,
        public_name: "Local Ivory",
        slug: "ivory",
      },
      {
        internal_name: `${prefix} fabric moss`,
        is_premium: true,
        public_name: "Local Moss",
        slug: "moss",
      },
    ],
    prefix,
    sofas: [
      {
        depth_cm: 96,
        fabric_slugs: ["graphite", "ivory", "moss"],
        footprint_type: "straight",
        height_cm: 82,
        internal_name: `${prefix} sofa alpha`,
        length_cm: 220,
        public_description: "Local seed sofa for admin catalog testing.",
        public_name: "Local Sofa Alpha",
        shopify_order_url: "https://shopify.example/products/local-sofa-alpha",
        slug: `${prefix}-sofa-alpha`,
        source_fabric_slug: "graphite",
        tag_slugs: [`${prefix}-test`, `${prefix}-modular`],
        visual_positions: [
          {
            admin_label: "Front",
            public_label: "Front",
      ...
      },
      {
        ...
        footprint_type: "straight",
        height_cm: 84,
        internal_name: `${prefix} sofa beta`,
        length_cm: 248,
        public_description: "Second local seed sofa for repeated manual testing.",
        public_name: "Local Sofa Beta",
        shopify_order_url: "https://shopify.example/products/local-sofa-beta",
        slug: `${prefix}-sofa-beta`,
        source_fabric_slug: "graphite",
        tag_slugs: [`${prefix}-test`, `${prefix}-premium`],
        visual_positions: [
          {
            admin_label: "Front",
            public_label: "Front",
    ...
    tags: [
      ...
      {
        public_label: "Local Modular",
        slug: `${prefix}-modular`,
      },
      {
        public_label: "Local Premium",
        slug: `${prefix}-premium`,
      },
    ],
  };
