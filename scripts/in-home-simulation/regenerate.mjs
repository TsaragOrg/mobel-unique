...
function fail(message, exitCode = 1) {
  console.error(`FAIL sim:regenerate: ${message}`);
...
async function main() {
  ...
  if (!args.jobId) {
    fail(
      "missing job id. Usage: pnpm sim:regenerate -- <job_id> [--wall-width N --wall-height N]",
      2
    );
  }
  ...
  info(`Requesting regeneration for job ${args.jobId}`);
  const msgId = await callRpc({
    supabaseUrl,
    serviceRoleKey,
    name: "request_in_home_simulation_regeneration",
    body: {
      job_id: args.jobId,
  ...
  info(`PASS sim:regenerate: queued placement msg_id=${msgId}`);
}
