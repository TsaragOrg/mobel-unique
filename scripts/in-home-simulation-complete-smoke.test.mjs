...
const scriptPath = fileURLToPath(
  new URL("./in-home-simulation-complete-smoke.mjs", import.meta.url)
);
const TEST_TIMEOUT_MS = 45000;

function createFakePsql() {
  const cwd = mkdtempSync(
    join(tmpdir(), "mobel-in-home-simulation-complete-smoke-test-")
  );
...
describe("in-home simulation complete smoke script", () => {
  it(
    "passes when psql returns no complete contract failures",
    async () => {
      const result = await runSmoke({
        IN_HOME_SIMULATION_COMPLETE_SMOKE_PSQL: createFakePsql()
      });

      expect(result.status).toBe(0);
      expect(result.stdout).toContain(
        "PASS in-home simulation complete smoke"
      );
    },
    TEST_TIMEOUT_MS
  );
  ...
  it(
    "fails clearly when psql reports complete contract failures",
    async () => {
      const result = await runSmoke({
        FAKE_PSQL_STDOUT:
          "complete did not transition the job to awaiting_dimensions with the expected fields\ncomplete accepted a worker_identifier that does not match the current claim\n",
        IN_HOME_SIMULATION_COMPLETE_SMOKE_PSQL: createFakePsql()
      });

      expect(result.status).toBe(1);
      expect(result.stderr).toContain(
        "FAIL in-home simulation complete smoke"
      );
      expect(result.stderr).toContain(
        "complete did not transition the job to awaiting_dimensions"
      );
      expect(result.stderr).toContain(
        "complete accepted a worker_identifier that does not match the current claim"
      );
  ...
  it(
    ...
    async () => {
      const result = await runSmoke({
        ...
        IN_HOME_SIMULATION_COMPLETE_SMOKE_PSQL: createFakePsql(),
        IN_HOME_SIMULATION_COMPLETE_SMOKE_TIMEOUT_MS: "50"
      });
      ...
      expect(result.stdout).not.toContain(
        "SKIP in-home simulation complete smoke"
      );
    },
    TEST_TIMEOUT_MS
  );
  ...
  it(
    ...
    async () => {
      const result = await runSmoke({
        ...
        IN_HOME_SIMULATION_COMPLETE_SMOKE_DB_URL:
