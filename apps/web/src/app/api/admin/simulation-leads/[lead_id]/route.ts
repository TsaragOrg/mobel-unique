...
import { handleDeleteAdminSimulationLeadRequest } from "../../../../../lib/admin-simulation-leads-route-handlers";
...
interface AdminSimulationLeadRouteContext {
  params: Promise<{
    lead_id: string;
  }>;
}
...
export async function DELETE(
  request: Request,
  context: AdminSimulationLeadRouteContext,
) {
  try {
    const adminInput = await readAdminInput();
    const { lead_id: leadId } = await context.params;

    return handleDeleteAdminSimulationLeadRequest({
      ...adminInput,
      leadId,
      request,
    });
