...
import { handleListSofaFabricsRequest } from "../../../../../../lib/admin-catalog-route-handlers";
...
interface SofaFabricsRouteContext {
  params: Promise<{
    sofa_id: string;
  }>;
}

export async function GET(_request: Request, context: SofaFabricsRouteContext) {
  try {
    const adminInput = await readAdminInput();
    const { sofa_id: sofaId } = await context.params;

    return handleListSofaFabricsRequest({
      ...adminInput,
      sofaId,
    });
  } catch {
