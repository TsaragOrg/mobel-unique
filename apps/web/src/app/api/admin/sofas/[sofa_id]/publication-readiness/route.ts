...
import { handleGetSofaPublicationReadinessRequest } from "../../../../../../lib/admin-catalog-route-handlers";
...
interface PublicationReadinessRouteContext {
  params: Promise<{
    sofa_id: string;
  }>;
}
...
export async function GET(
  _request: Request,
  context: PublicationReadinessRouteContext
) {
  try {
    const adminInput = await readAdminInput();
    const { sofa_id: sofaId } = await context.params;

    return handleGetSofaPublicationReadinessRequest({
      ...adminInput,
      sofaId
    });
  } catch {
