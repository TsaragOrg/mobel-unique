...
interface SofaRouteContext {
  params: Promise<{
    sofa_id: string;
  }>;
}

export async function GET(_request: Request, context: SofaRouteContext) {
  try {
    const adminInput = await readAdminInput();
    const { sofa_id: sofaId } = await context.params;

    return handleGetSofaRequest({
      ...adminInput,
      sofaId
    });
  } catch {
...
export async function PATCH(request: Request, context: SofaRouteContext) {
  try {
    const adminInput = await readAdminInput();
    const { sofa_id: sofaId } = await context.params;

    return handleUpdateSofaRequest({
      ...adminInput,
      request,
      sofaId
    });
