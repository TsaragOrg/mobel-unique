...
import { handleArchiveFabricRequest } from "../../../../../../lib/admin-catalog-route-handlers";
...
interface FabricArchiveRouteContext {
  params: Promise<{
    fabric_id: string;
  }>;
}
...
export async function POST(
  _request: Request,
  context: FabricArchiveRouteContext,
) {
  try {
    const adminInput = await readAdminInput();
    const { fabric_id: fabricId } = await context.params;

    return handleArchiveFabricRequest({
      ...adminInput,
      fabricId,
    });
  } catch {
