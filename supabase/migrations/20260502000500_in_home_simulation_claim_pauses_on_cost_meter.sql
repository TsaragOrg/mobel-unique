-- SPEC-0015 PLAN-0039: claim RPCs short-circuit on cost-meter pause.
--
-- Adds a `public.simulation_cost_meter_paused()` helper that
-- returns true when today's row in `public.simulation_cost_meter`
-- has `worker_paused = true`. Each in-home simulation claim RPC
-- (Stage 1 queue-driven, Stage 1 by id, Stage 2 by id) calls the
-- helper before picking a candidate row. When paused, the function
-- returns zero rows and the worker dequeues nothing for the rest
-- of the day, even though new jobs continue to be created at the
-- API level.
--
-- The function bodies otherwise match the previous PLAN-0038 /
-- PLAN-0011 versions exactly: same parameters, same return shape,
-- same status transitions and counter increments. Only the early
-- return on paused meter is new.
...
as $$
  select exists (
    ...
    where cost_date = (now() at time zone 'utc')::date
      and worker_paused = true
  );
$$;
